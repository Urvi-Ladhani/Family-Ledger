...
export default function Dashboard() {
  ...
  // Extract text from image using Tesseract directly using browser File buffer
  const extractTextFromImage = async (file: File): Promise<string> => {
    try {
      ...
      // Passing raw file handles chunks correctly and stops "truncated file" WebAssembly crashes
  ...
  // Handler for manual submission form
  ...
  const { getRootProps, getInputProps } = useDropzone({
    onDrop: async (acceptedFiles) => {
      ...
      try {
        ...
        let extracted = { amount: null as number | null, currency: 'USD', date: today, description: file.name, category: 'Other', confidence: 0, rawText: '' };

        if (file.type.startsWith('image/')) {
          extractedText = await extractTextFromImage(file);
          const parsedExpense = extractExpenseData(extractedText);
          extracted = { ...parsedExpense, date: parsedExpense.date || today, description: parsedExpense.description || file.name };
        ...
        // Explicit contentType header avoids file uploads freezing on storage buckets
  ...
  const filteredBills = useMemo(() => bills.filter((b) => (!selectedRange.start && !selectedRange.end) ? true : isInRange(b.extracted_date || b.created_at, selectedRange.start, selectedRange.end)), [bills, selectedRange]);
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <div {...getRootProps()} className="border-2 border-dashed border-blue-300 rounded-lg p-8 text-center cursor-pointer hover:bg-blue-50 transition-colors">
              <input {...getInputProps()} />
              <p className="text-gray-600">{uploading ? 'Reading OCR and saving expense...' : 'Drag and drop a bill image/PDF or click to browse'}</p>
            </div>
            {ocrSummary && (
